...
func NewRouter(prep port.PreparationService, auth port.AuthService, classProfile port.ClassProfileService, allowedOrigins []string) stdhttp.Handler {
	...
	mux.Handle("GET /api/programmations", protected(stdhttp.HandlerFunc(handler.ListProgrammations)))
	mux.Handle("GET /api/programmations/{id}", protected(stdhttp.HandlerFunc(handler.GetProgrammation)))
	mux.Handle("DELETE /api/programmations/{id}", protected(stdhttp.HandlerFunc(handler.DeleteProgrammation)))
